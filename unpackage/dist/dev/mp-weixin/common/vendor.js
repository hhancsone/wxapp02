...
const toDisplayString$1 = (val) => {
  return isString$1(val) ? val : val == null ? "" : isArray$1(val) || isObject$4(val) && (val.toString === objectToString$1 || !isFunction$1(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
const replacer = (_key, val) => {
  ...
  } else if (isObject$4(val) && !isArray$1(val) && !isPlainObject$1(val)) {
    return String(val);
  }
  return val;
};
...
function patchMPEvent(event, instance) {
  if (event.type && event.target) {
    ...
    if (isPlainObject$1(event.detail) && hasOwn$3(event.detail, "checked") && !hasOwn$3(event.detail, "value")) {
...
function validateProtocol(name, data, protocol, onFail) {
  ...
  for (const key in protocol) {
    const errMsg = validateProp(key, data[key], protocol[key], !hasOwn$3(data, key));
...
function initWrapper(protocols2) {
  ...
  return function wrapper(methodName, method) {
    const hasProtocol = hasOwn$3(protocols2, methodName);
...
function initRuntimeSocketService() {
  const hosts = "26.13.126.165,192.168.56.1,192.168.179.1,192.168.154.1,192.168.1.103,127.0.0.1";
  const port = "8090";
  const id = "mp-weixin_q0YlQQ";
...
const b = true, E = "mp-weixin", A = T(define_process_env_UNI_SECURE_NETWORK_CONFIG_default), P = E, C = T('{"address":["127.0.0.1","26.13.126.165","192.168.56.1","192.168.179.1","192.168.154.1","192.168.1.103"],"servePort":7001,"debugPort":9001,"initialLaunchType":"local","skipFiles":["<node_internals>/**","C:/Users/黄文达/Desktop/HBuilderX/plugins/unicloud/**/*.js"]}'), O = T('[{"provider":"aliyun","spaceName":"hwd","spaceId":"mp-cd26c9a6-c5a5-4c83-a96c-85ff377a9b00","clientSecret":"nNcn8XBraEkrGNHk8Up59A==","endpoint":"https://api.next.bspapp.com"}]') || [];
let N = "";
try {
  N = "__UNI__CFEB179";
} catch (e2) {
...
const { loginPage: vs, routerNeedLogin: Ss, resToLogin: Ts, needLoginPage: bs, notNeedLoginPage: Es, loginPageInTabBar: ks$1 } = function({ pages: t2 = [], subPackages: n2 = [], uniIdRouter: s2 = {}, tabBar: r2 = {} } = pagesJson) {
