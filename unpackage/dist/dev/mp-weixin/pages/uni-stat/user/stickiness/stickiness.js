  ...
  methods: {
    ...
    getChartData(query, field = this.field, name2 = this.fields.find((f) => f._id === this.field).name) {
      ...
      db.collection("uni-stat-loyalty-result").where(query).groupBy("appid").groupField(groupField).orderBy("start_time", "asc").get({
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/user/stickiness/stickiness.vue:254", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    getTabelData(query) {
      ...
      db.collection("uni-stat-loyalty-result").where(query).groupBy("appid").groupField(groupField).orderBy("start_time", "asc").get({
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/user/stickiness/stickiness.vue:331", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    ...
    getChannelData(appid, platform_id) {
      ...
      db.collection(channelTemp, platformTemp).orderBy("platform_id", "asc").get().then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/user/stickiness/stickiness.vue:385", err);
      }).finally(() => {
      });
    }
  }
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    p: common_vendor.sr("version-select", "26917e54-6"),
...
//# sourceMappingURL=../../../../../.sourcemap/mp-weixin/pages/uni-stat/user/stickiness/stickiness.js.map
