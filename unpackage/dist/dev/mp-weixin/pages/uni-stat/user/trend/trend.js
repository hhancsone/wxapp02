...
const pages_uniStat_user_trend_fieldsMap = require("./fieldsMap.js");
const _sfc_main = {
  data() {
    return {
      fieldsMap: pages_uniStat_user_trend_fieldsMap.fieldsMap,
      query: {
        dimension: "day",
        appid: "",
        platform_id: "",
      ...
      tableData: [],
      panelData: pages_uniStat_user_trend_fieldsMap.fieldsMap.filter((f) => f.hasOwnProperty("value")),
      chartData: {},
  ...
  computed: {
    chartTabs() {
      const tabs = [];
      pages_uniStat_user_trend_fieldsMap.fieldsMap.forEach((item) => {
        const {
          field: _id,
          title: name
        } = item;
  ...
  methods: {
    ...
    getChartData(query, field = this.chartTab, name = "新增用户") {
      ...
      db.collection("uni-stat-result").where(query).field(`${js_sdk_uniStat_util.stringifyField(pages_uniStat_user_trend_fieldsMap.fieldsMap, field)}, start_time`).groupBy("start_time").groupField(js_sdk_uniStat_util.stringifyGroupField(pages_uniStat_user_trend_fieldsMap.fieldsMap, field)).orderBy("start_time", "asc").get({
        getCount: true
      }).then((res) => {
        const {
          count,
        ...
        let mapper = pages_uniStat_user_trend_fieldsMap.fieldsMap.filter((f) => f.field === field);
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/user/trend/trend.vue:319", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    getTabelData(query) {
      ...
      this.loading = true;
      const db = common_vendor.tr.database();
      db.collection("uni-stat-result").where(query).field(js_sdk_uniStat_util.stringifyField(pages_uniStat_user_trend_fieldsMap.fieldsMap)).groupBy("start_time").groupField(js_sdk_uniStat_util.stringifyGroupField(pages_uniStat_user_trend_fieldsMap.fieldsMap)).orderBy("start_time", "desc").skip((pageCurrent - 1) * this.options.pageSize).limit(this.options.pageSize).get({
        getCount: true
      }).then((res) => {
        const {
          count,
        ...
        for (const item of data) {
          ...
          js_sdk_uniStat_util.mapfields(pages_uniStat_user_trend_fieldsMap.fieldsMap, item, item);
        }
        this.tableData = [];
        this.options.total = count;
        this.tableData = data;
      }).catch((err) => {
        common_vendor.index.__f__("error", "at pages/uni-stat/user/trend/trend.vue:363", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    getPanelData() {
      ...
      let query = js_sdk_uniStat_util.stringifyQuery(cloneQuery, null, ["uni_platform"]);
      const db = common_vendor.tr.database();
      db.collection("uni-stat-result").where(query).field(`${js_sdk_uniStat_util.stringifyField(pages_uniStat_user_trend_fieldsMap.fieldsMap)},stat_date`).groupBy("appid").groupField(js_sdk_uniStat_util.stringifyGroupField(pages_uniStat_user_trend_fieldsMap.fieldsMap)).orderBy("stat_date", "desc").get().then((res) => {
        const item = res.result.data[0];
        item && (item.total_users = 0);
        this.panelData = [];
        this.panelData = js_sdk_uniStat_util.mapfields(pages_uniStat_user_trend_fieldsMap.fieldsMap, item);
    ...
    getChannelData(appid, platform_id) {
      ...
      db.collection(channelTemp, platformTemp).orderBy("platform_id", "asc").get().then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/user/trend/trend.vue:446", err);
      }).finally(() => {
      });
    }
  }
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    r: common_vendor.sr("version-select", "26faa7d0-7"),
