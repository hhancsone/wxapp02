...
const pages_uniStat_device_retention_fieldsMap = require("./fieldsMap.js");
const _sfc_main = {
  data() {
    return {
      query: {
  ...
  computed: {
    fieldsMap() {
      ...
      return pages_uniStat_device_retention_fieldsMap.fieldsFactory(maps);
    },
    fieldName() {
      let name = "";
  ...
  methods: {
    ...
    getChartData(query, key = this.key, name = "访问人数") {
      ...
      this.createStr("device_count", [key], [this.field]);
      ...
      db.collection("uni-stat-result").where(query).field(`${this.stringifyField(this.fieldsMap, `d_${key}`)}, start_time`).groupBy("start_time").groupField(js_sdk_uniStat_util.stringifyGroupField(this.fieldsMap, `d_${key}`)).orderBy("start_time", "asc").get({
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/device/retention/retention.vue:313", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    getTabelData(query) {
      ...
      db.collection("uni-stat-result").where(query).field(this.stringifyField(this.fieldsMap)).groupBy("start_time").groupField(js_sdk_uniStat_util.stringifyGroupField(this.fieldsMap)).orderBy("start_time", "desc").skip((pageCurrent - 1) * this.options.pageSize).limit(this.options.pageSize).get({
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/device/retention/retention.vue:353", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    ...
    getChannelData(appid, platform_id) {
      ...
      db.collection(channelTemp, platformTemp).orderBy("platform_id", "asc").get().then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/device/retention/retention.vue:407", err);
      }).finally(() => {
      });
    }
  }
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    p: common_vendor.sr("version-select", "680b3c46-6"),
      ...
      return {
        a: common_vendor.f($options.fieldsMap, (mapper, index, i1) => {
          return common_vendor.e({
            ...
            e: "680b3c46-15-" + i0 + "-" + i1 + "," + ("680b3c46-14-" + i0),
            f: common_vendor.p({
              align: "center"
            })
          } : {}, {
...
//# sourceMappingURL=../../../../../.sourcemap/mp-weixin/pages/uni-stat/device/retention/retention.js.map
