...
const pages_uniStat_device_stickiness_fieldsMap = require("./fieldsMap.js");
const _sfc_main = {
  data() {
    return {
      fieldsMap: pages_uniStat_device_stickiness_fieldsMap.fieldsMap,
      query: {
        // dimension: "hour",
        appid: "",
        platform_id: "",
      ...
      currentDateTab: 1,
      tableData: [],
      panelData: pages_uniStat_device_stickiness_fieldsMap.fieldsMap.filter((f) => f.hasOwnProperty("value")),
      chartData: {},
  ...
  methods: {
    ...
    getChartData(query, field = this.field, name2 = this.fields.find((f) => f._id === this.field).name) {
      ...
      db.collection("uni-stat-loyalty-result").where(query).groupBy("appid").groupField(groupField).orderBy("start_time", "asc").get({
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/device/stickiness/stickiness.vue:256", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    getTabelData(query) {
      ...
      db.collection("uni-stat-loyalty-result").where(query).groupBy("appid").groupField(groupField).orderBy("start_time", "asc").get({
        ...
        let users = rows.filter((row) => row.visit_devices).map((row) => row.visit_devices);
        ...
        for (const item of tableData) {
          js_sdk_uniStat_util.mapfields(pages_uniStat_device_stickiness_fieldsMap.fieldsMap, item, item);
        }
        this.tableData = [];
        this.tableData = tableData;
      }).catch((err) => {
        common_vendor.index.__f__("error", "at pages/uni-stat/device/stickiness/stickiness.vue:333", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    ...
    getChannelData(appid, platform_id) {
      ...
      db.collection(channelTemp, platformTemp).orderBy("platform_id", "asc").get().then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/device/stickiness/stickiness.vue:387", err);
      }).finally(() => {
      });
    }
  }
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    p: common_vendor.sr("version-select", "b2f96e42-6"),
    ...
    s: common_vendor.o(($event) => $data.type = $event),
    t: common_vendor.p({
      type: "boldLine",
      tabs: $data.types,
...
//# sourceMappingURL=../../../../../.sourcemap/mp-weixin/pages/uni-stat/device/stickiness/stickiness.js.map
