...
const pages_uniStat_pageRes_fieldsMap = require("./fieldsMap.js");
const _sfc_main = {
  data() {
    return {
      fieldsMap: pages_uniStat_pageRes_fieldsMap.fieldsMap,
      query: {
        dimension: "day",
        appid: "",
        platform_id: "",
      ...
      currentDateTab: 1,
      tableData: [],
      panelData: pages_uniStat_pageRes_fieldsMap.fieldsMap.filter((f) => f.hasOwnProperty("value")),
      queryId: "",
      updateValue: "",
      channelData: [],
      errorMessage: ""
  ...
  methods: {
    ...
    getTableData(query) {
      ...
      const subTableTemp = db.collection("uni-stat-page-result").where(query).getTemp();
      db.collection(subTableTemp, mainTableTemp).field(
        `${js_sdk_uniStat_util.stringifyField(pages_uniStat_pageRes_fieldsMap.fieldsMap)}, stat_date, page_id`
      ).groupBy("page_id").groupField(js_sdk_uniStat_util.stringifyGroupField(pages_uniStat_pageRes_fieldsMap.fieldsMap)).orderBy("visit_times", "desc").skip((pageCurrent - 1) * this.options.pageSize).limit(this.options.pageSize).get({
        getCount: true
      }).then((res) => {
        const {
          count,
        ...
        for (const item of data) {
          ...
          js_sdk_uniStat_util.mapfields(pages_uniStat_pageRes_fieldsMap.fieldsMap, item, item);
          this.tableData.push(item);
        }
      }).catch((err) => {
        common_vendor.index.__f__("error", "at pages/uni-stat/page-res/page-res.vue:248", err);
      }).finally(() => {
        this.loading = false;
      });
    },
    getPanelData(query = js_sdk_uniStat_util.stringifyQuery(this.query, null, ["uni_platform"])) {
      ...
      this.errorMessage = "";
      const db = common_vendor.tr.database();
      db.collection("uni-stat-page-result").where(query).field(js_sdk_uniStat_util.stringifyField(pages_uniStat_pageRes_fieldsMap.fieldsMap)).groupBy("appid").groupField(js_sdk_uniStat_util.stringifyGroupField(pages_uniStat_pageRes_fieldsMap.fieldsMap)).orderBy("start_time", "desc ").get().then((res) => {
        const items = res.result.data[0];
        this.panelData = [];
        this.panelData = js_sdk_uniStat_util.mapfields(pages_uniStat_pageRes_fieldsMap.fieldsMap, items);
      });
    },
    ...
    editName(value) {
      const db = common_vendor.tr.database();
      db.collection("uni-stat-pages").where({
        path: this.queryId
      }).update({
        title: value
      }).then((res) => {
    ...
    getChannelData(appid, platform_id) {
      ...
      db.collection(channelTemp, platformTemp).orderBy("platform_id", "asc").get().then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/uni-stat/page-res/page-res.vue:361", err);
      }).finally(() => {
      });
    }
  }
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    o: common_vendor.sr("version-select", "24b12a56-6"),
      ...
      return {
        a: common_vendor.f($data.fieldsMap, (mapper, index, i1) => {
          return common_vendor.e({
            ...
            c: common_vendor.o(($event) => $options.inputDialogToggle(item.path, item.title), index),
            d: "24b12a56-13-" + i0 + "-" + i1 + "," + ("24b12a56-12-" + i0 + "-" + i1),
            e: common_vendor.p({
              type: "compose",
              color: "#2979ff",
              size: "25"
            ...
            f: mapper.field,
            g: "24b12a56-12-" + i0 + "-" + i1 + "," + ("24b12a56-11-" + i0)
          } : {
            h: common_vendor.t(item[mapper.field] !== void 0 ? item[mapper.field] : "-"),
            i: mapper.field,
            j: "24b12a56-14-" + i0 + "-" + i1 + "," + ("24b12a56-11-" + i0),
            k: common_vendor.p({
    ...
    z: common_vendor.sr("inputClose", "24b12a56-17,24b12a56-16"),
    A: common_vendor.o($options.editName),
    ...
    D: common_vendor.sr("inputDialog", "24b12a56-16"),
    E: common_vendor.p({
      type: "dialog",
      maskClick: true
    })
...
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pages/uni-stat/page-res/page-res.js.map
